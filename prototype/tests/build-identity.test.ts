...
function post(origin: string, stream: boolean) {
  return fetch(`${origin}/v1/chat/completions`, {
    method: "POST", headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
    body: JSON.stringify({ model: "swe-2-high", stream, messages: [{ role: "user", content: "fixture" }] }),
  });
}
...
test("identifies the bridge build in JSON completions, every SSE chunk, and the health endpoint", async () => {
  ...
  await serve(async origin => {
    const fingerprint = `devin-bridge-${BRIDGE_VERSION}`;
