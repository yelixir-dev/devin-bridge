// Local replacement for the upstream utility dependency; codec logic is unchanged.
...
class Writer {
	...
	finish(): Uint8Array {
		...
		if (this.#chunks.length === 1 && first) return first;
...
class Reader {
	...
	int64(): bigint {
		...
		while (this.pos < this.len) {
			const byte = this.buf[this.pos++];
			if (byte === undefined) throw new Error("Unexpected end of protobuf 64-bit varint");
